...
export async function GET(request: Request, { params }: Params): Promise<Response> {
  return withApi(request, {}, async ({ supabase }) => {
...
export async function PATCH(request: Request, { params }: Params): Promise<Response> {
  return withApi(request, { write: true }, async ({ supabase, user, jwt }) => {
    const { id } = await params
...
// con su subárbol (recuperable desde la web). RLS (editor+) gatea. Requiere scope
// `read_write`.
export async function DELETE(request: Request, { params }: Params): Promise<Response> {
  return withApi(request, { write: true }, async ({ supabase }) => {
    const { id } = await params
