...
async function subtreeIds(
  ...
  id: string,
  trashed: boolean,
): Promise<{ teamId: string; ids: string[] } | null> {
  ...
  let q = supabase.from('documents').select('id, parent_id').eq('team_id', teamId)
  ...
  return { teamId, ids: [id, ...collectDescendantIds(list, id)] }
}
...
export async function softDeleteDoc(supabase: SupabaseClient, id: string): Promise<TrashResult> {
  const sub = await subtreeIds(supabase, id, false)
  if (!sub) return { ok: false, status: 404, error: 'Documento no encontrado o sin acceso.' }
...
export async function restoreDoc(supabase: SupabaseClient, id: string): Promise<TrashResult> {
  const sub = await subtreeIds(supabase, id, true)
  if (!sub) return { ok: false, status: 404, error: 'Documento no encontrado o sin acceso.' }
...
export async function purgeDoc(supabase: SupabaseClient, id: string): Promise<TrashResult> {
  ...
  return { ok: true, status: 200, count: ids.length, teamId: sub.teamId, ids }
}
